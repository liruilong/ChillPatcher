...
### 🔗 本地集成模块
- **🌐 本地 HTTP 总线**：提供可配置的 localhost 集成入口，支持 Token 鉴权
- **🧰 本地工具接入**：适合本地自动化脚本、桌面助手、直播/语音工作流、快捷键面板、Stream Deck 等可信调用方
- **🧩 模块化扩展**：功能模块可通过实现 `ILocalIntegrationHandler` 注册自己的本地 HTTP 路由
- **🧵 主线程派发**：总线负责把已接受的请求排队回 Unity 主线程执行，避免模块自行处理跨线程问题
...
├── ChillPatcher.Module.LocalIntegration/  ← 本地集成总线模块
...
- **[本地集成模块](ChillPatcher.Module.LocalIntegration/README.md)** - 本地 HTTP 总线和模块处理器扩展示例
